...
class AddressBookService {
  ...
  def compareAge(person1: String, person2: String, fileName: String): String = {
    val ls = convertFileToList(fileName, readFile)
    val dobPerson1 = convertToDate(ls.filter(l => l.toUpperCase.contains(person1.toUpperCase())).headOption.getOrElse(throw new RuntimeException(s"No person of name $person1 could be found")).split(", ")(2)).getOrElse(throw new RuntimeException("Incorrect date of birth format in text file"))
    val dobPerson2 = convertToDate(ls.filter(l => l.toUpperCase.contains(person2.toUpperCase())).headOption.getOrElse(throw new RuntimeException(s"No person of name $person2 could be found")).split(", ")(2)).getOrElse(throw new RuntimeException("Incorrect date of birth format in text file"))
